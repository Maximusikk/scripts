local function createTutorialGuiQuests(context)
	...
	local startEquipBestStep = context.startEquipBestStep
	...
	local function startGoToFinalWallStep()
		...
		arrows.pointArrowToFinalWall(function()
	...
	local function startGoToGrassBlockStep()
		...
		arrows.pointArrowToGrassBlock(function()
	...
	local function startGoToWallStep()
		...
		arrows.pointArrowToWall(function()
	...
	local function startGoToSecondWallStep()
		...
		arrows.pointArrowToSecondWall(function()
	...
	local function startGoToUpgradeMachineStep()
		...
		arrows.pointArrowToUpgradeMachine(function()
	...
	local function startBarnPhase1()
		...
		arrows.pointArrowToBarnBlock(function()
	...
	local function startGoToThirdWallStep()
		...
		arrows.pointArrowToThirdWall(function()
			showNotify("Press E on PC or tap the E button on mobile\nto buy and open the new area!", 0)
		end)
	end
	...
	local function startGoToHoverMeshStep()
		...
		arrows.pointArrowToHoverMesh(function()
	...
	startUpgradeButtonStep = function()
		...
		local function tryAttachArrow(): boolean
			...
			upgradeArrowGui = arrows.createGuiArrowLabel("UpgradeArrowHint", 120)
			showNotify("Tap this button to buy your first speed upgrade!", 0)

			upgradeArrowConn = arrows.followGuiObject(upgradeArrowGui, upgradeBtn, -10, 4, 6)
	...
	local function showBoostActivateButtonStep()
		...
		cleanupBoostArrow()
		boostArrowGui = arrows.createGuiArrowLabel("BoostActivateArrowHint", 90)
		boostArrowConn = arrows.followGuiObject(boostArrowGui, boostBtn, -10, 4, 6)
	...
	showBoostSelectTabStep = function()
		...
		cleanupBoostArrow()
		boostArrowGui = arrows.createGuiArrowLabel("BoostsTabArrowHint", 80)
		boostArrowConn = arrows.followGuiObject(boostArrowGui, tabBtn, -10, 4, 6)
	...
	local function showBoostInventoryStep()
		...
		cleanupBoostArrow()
		boostArrowGui = arrows.createGuiArrowLabel("BoostInventoryArrowHint", 80)
		boostArrowConn = arrows.followGuiObject(boostArrowGui, inventoryBtn, -10, 4, 6)
	...
	handleWall2Purchased = function()
		...
		task.delay(1, function()
			...
			arrows.pointArrowToFirstGems_Location3(function()
	...
	handleFinalWallPurchased = function()
		...
		task.delay(8, function()
			...
			local notifyLabel = tutorialGui:FindFirstChild("NotifyText")
	...
	local function handleWallEvent()
	...
	local function beginInteractiveTutorial()
		...
		arrows.pointArrowToFirstCoin(function()
			showNotify("Great! Now click the coin to collect your first coins!", 0)
		end)
	end

	local function handleHatchedNft(hatchedUuid: string?, inventoryModule)
		...
		if not quest2Active
			...
			and not inventoryModule.isInventoryStepDone()
			and not inventoryModule.isNftClickStepDone()
		then
			arrows.destroyArrow()

			inventoryModule.setFirstNftUuid(hatchedUuid)
			warn("[Tutorial] First hatch, firstNftUuid =", hatchedUuid)

			showNotify("Nice! You opened the block and got your first NFT!", 0)

			task.delay(1.5, function()
				if not inventoryModule.isInventoryStepDone() and not inventoryModule.isNftClickStepDone() then
					inventoryModule.showInventoryHint()
				end
			end)
		...
		if wantExtraBlocks then
			...
			inventoryModule.setEquipReminderCooldown(tick() + 8)
			inventoryModule.resetEquipReminder()
		...
		if hatchedUuid and hatchedUuid ~= "" then
			if not wantExtraBlocks
				...
				and not inventoryModule.isEquipBestStepActive()
				and not inventoryModule.isInventoryStepActive()
				and not inventoryModule.isNftClickStepActive()
			then
				inventoryModule.showEquipReminderForUuid(hatchedUuid)
			end
		end
	end
	...
	return {
		...
		beginInteractiveTutorial = beginInteractiveTutorial,
		handleWallEvent = handleWallEvent,
		startBoostTutorial = startBoostTutorial,
		handleHatchedNft = handleHatchedNft,
		handleHoverboardPurchased = handleHoverboardPurchased,
		isQuest1Finished = function()
			return quest1Finished
		end,
		isQuest2Active = function()
			return quest2Active
		end,
		isQuest2Finished = function()
			return quest2Finished
		end,
		isQuest3Active = function()
			return quest3Active
		end,
		isQuest3Finished = function()
			return quest3Finished
		end,
	}
end

return createTutorialGuiQuests
