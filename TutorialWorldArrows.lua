...
function WorldArrows.new(params)
	...
	local worldRoot = params.worldRoot
	...
	local function pointArrowToUpgradeMachine(onReached)
		...
		for _, obj in ipairs(worldRoot:GetDescendants()) do
			if obj:IsA("BasePart") and obj.Name:lower():find("upgrade") then
				targetPart = obj
				break
			end
		end
		...
		if not targetPart then
			warn("[Tutorial] Upgrade machine base part not found")
			return
		end
		...
		local arrow = arrowTemplate:Clone()
		arrow.Name = "TutorialArrowRuntime_Upgrade"
		arrow.Anchored = true
		arrow.CanCollide = false
		arrow.Parent = Workspace
		arrowPart = arrow
	...
	local function pointArrowToBarnBlock(onReachedBlock)
		...
		local barnFolder = worldRoot:FindFirstChild("BarnBlock")
		if not barnFolder then
			for _, obj in ipairs(worldRoot:GetDescendants()) do
				if obj.Name == "BarnBlock" then
					barnFolder = obj
					break
				end
			end
		...
		if not barnFolder then
			warn("[Tutorial] BarnBlock folder not found in PixelWorld1")
			return
		end

		local barnPart: BasePart? = nil
		for _, obj in ipairs(barnFolder:GetDescendants()) do
			if obj:IsA("BasePart") then
				barnPart = obj
				break
			end
		...
		if not barnPart then
			warn("[Tutorial] BarnBlock BasePart not found")
			return
		end

		local targetPos = barnPart.Position
		...
		local sizeNearPlayer = arrow.Size
		local sizeOnBarn = sizeNearPlayer * 1.7
		...
		task.spawn(function()
			while arrowPart and arrowPart.Parent and barnPart.Parent and myToken == arrowToken do
				local hrp = character:FindFirstChild("HumanoidRootPart")
				if not hrp then break end

				local hrpPos = hrp.Position
				targetPos = barnPart.Position
				...
				if not reached then
					...
					if arrowPart.Size ~= sizeOnBarn then
						arrowPart.Size = sizeOnBarn
					end

					local t = tick() - startTime
					local yOffset = barnOffsetY + math.sin(t * 2) * 0.5
					local pos = targetPos + Vector3.new(0, yOffset, 0)
					arrowPart.CFrame = CFrame.new(pos, targetPos) * ARROW_ROT_OFFSET
				end
	...
	local function pointArrowToThirdWall(onReachedWall)
		...
		local wallFolder = worldRoot:FindFirstChild("Location3Wall")
		if not wallFolder then
