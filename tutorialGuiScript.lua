...
local GuiUtils = require(script:WaitForChild("TutorialGuiUtils"))
local WorldArrows = require(script:WaitForChild("TutorialWorldArrows"))
local InventoryTracker = require(script:WaitForChild("TutorialInventoryTracker"))
...
local baseMainGui: ScreenGui? = playerGui:FindFirstChild("MainGui")
if baseMainGui and baseMainGui.DisplayOrder >= tutorialGui.DisplayOrder then
	baseMainGui.DisplayOrder = tutorialGui.DisplayOrder - 1
end

local skipBtn = mainFrame:WaitForChild("SkipBtn")

local guiUtils = GuiUtils.new({
	tutorialGui = tutorialGui,
	playerGui = playerGui,
	RunService = RunService,
})

local showNotify = guiUtils.showNotify
local hideNotify = guiUtils.hideNotify
local getMainGui = guiUtils.getMainGui
local findOpenInventoryMain = guiUtils.findOpenInventoryMain
local findInventoryContainer = guiUtils.findInventoryContainer
local createGuiArrowLabel = guiUtils.createGuiArrowLabel
local followGuiObject = guiUtils.followGuiObject
local ensureQuestUi = guiUtils.ensureQuestUi
local showQuest = guiUtils.showQuest
local hideQuest = guiUtils.hideQuest
local setQuestProgress = guiUtils.setQuestProgress
local setQuestVisible = guiUtils.setQuestVisible
...
local function getLastHatchedUuid(): string?
	return inventoryTracker.getLastHatchedUuid()
end

local function setLastHatchedUuid(uuid: string?)
	inventoryTracker.setLastHatchedUuid(uuid)
end
...
local isWallUnlocked = worldArrows.isWallUnlocked
