...
local GuiArrowHelpers     = require(script:WaitForChild("GuiArrowHelpers"))
local WorldArrowController = require(script:WaitForChild("WorldArrowController"))
...
local function handleGuideRobotNotification()
	showNotify(
...
local inventoryTracker = InventoryTracker.new({
	...
	onGuideRobot = handleGuideRobotNotification,
})
...
local function createGuiArrowLabel(name: string, z: number): TextLabel
	return GuiArrowHelpers.createGuiArrowLabel(tutorialGui, name, z)
end

local function followGuiObject(label: TextLabel, target: GuiObject, yOffset: number?, bobSpeed: number?, bobAmp: number?)
	return GuiArrowHelpers.followGuiObject(RunService, label, target, yOffset, bobSpeed, bobAmp)
end
...
local worldArrows = WorldArrowController.new({
	player = player,
	worldRoot = worldRoot,
	RunService = RunService,
	ReplicatedStorage = ReplicatedStorage,
	Workspace = Workspace,
	arrowRotationOffset = ARROW_ROT_OFFSET,
})
