-- ModuleScript: TutorialInventoryModule

local InventoryModule = {}

function InventoryModule.new(context)
	...
	local function startNftClickStep()
		...
		cleanupNftArrow()
		nftArrowGui = arrow.createGuiArrowLabel("NftArrowHint", 60)
		nftArrowConn = arrow.followGuiObject(nftArrowGui, clickArea, -10, 4, 6)
	...
	local function showInventoryHint()
		...
		cleanupInventoryArrow()
		inventoryArrowGui = arrow.createGuiArrowLabel("InventoryArrowHint", 50)
		inventoryArrowConn = arrow.followGuiObject(inventoryArrowGui, inventoryBtn, -10, 4, 6)
	...
	local function startEquipBestStep(onDone)
		...
		local function attachEquipBestButton()
			...
			cleanupEquipBestArrow()
			equipBestArrowGui = arrow.createGuiArrowLabel("EquipBestArrowHint", 95)
			equipBestArrowConn = arrow.followGuiObject(equipBestArrowGui, btn, -10, 4, 6)
		...
		localInvArrowGui = arrow.createGuiArrowLabel("EquipBest_InvArrow", 90)
		localInvArrowConn = arrow.followGuiObject(localInvArrowGui, inventoryBtn, -10, 4, 6)
	...
	local function showEquipReminderForUuid(uuid: string?)
		...
		inventoryArrowGui = arrow.createGuiArrowLabel("EquipInventoryArrowHint", 70)
		inventoryArrowConn = arrow.followGuiObject(inventoryArrowGui, inventoryBtn, -10, 4, 6)
		...
		inventoryBtnConn = inventoryBtn.MouseButton1Click:Connect(function()
			...
			task.delay(0.2, function()
				...
				nftArrowGui = arrow.createGuiArrowLabel("EquipNftArrowHint", 70)
				nftArrowConn = arrow.followGuiObject(nftArrowGui, clickArea, -10, 4, 6)
	...
	return {
		...
		isInventoryStepDone = function() return inventoryStepDone end,
		isNftClickStepDone = function() return nftClickStepDone end,
		isInventoryStepActive = function() return inventoryStepActive end,
		isNftClickStepActive = function() return nftClickStepActive end,
		isEquipBestStepActive = function() return equipBestStepActive end,
		setEquipReminderCooldown = function(seconds)
			equipReminderCooldownUntil = tick() + seconds
			equipReminderActive = false
		end,
	}
end
